...
class Chart(Canvas):
    ...
    #private function
    #to simplifie calculation a scheme of 0 to 100 is used for both height and width
    ...
    def draw(self):
        self.delete('all')
        self.select_clear()
        #self.create_line(1, 1, 20, 20)
        #(x1, y1) =self.__convert_x_y_to_pixel(25, 25)
        #(x2, y2) = self.__convert_x_y_to_pixel(100, 100)
        #self.create_line(x1,y1,x2,y2)
        #self.__draw_p_text(50,50,text="heloooo")
    ...
    def __draw_p_text(self , x, y ,text ):
        ...
        self.create_text(px, py ,text=text)
    ...
    def __draw_x_y_axis(self):
        ...
        self.__draw_p_line(Chart.padding_x_axis_left , Chart.padding_x_axis_bottom
                         ,Chart.padding_x_axis_right,Chart.padding_x_axis_bottom  )

        #place the line on the x axis
        i =0
        number_of_lines=5
        ...
        while i <number_of_lines+1 :
            self.__draw_p_line(x1,y1,x2,y2)
            self.__draw_p_text(x1, y1 - 3, text=str(x_val))
            x1+=x_increments
            x2=x1
        ...
        self.__draw_p_line(Chart.padding_y_axis_left, Chart.padding_y_axis_bottom \
                     , Chart.padding_y_axis_left, Chart.padding_y_axis_top)

        #place on the lines on the y axis
        i=0
        ...
        while i < number_of_lines + 1:
            self.__draw_p_line(x1, y1, x2, y2)
            self.__draw_p_text(4, y1 , text=str(y_val))
            y_val += y_val_increments
            y1+=y_increments
            y2=y1
            i+=1

    #clears all the data curently available in the queue
    def clear_data(self):
    ...
    #add a data point
    #this is how you stream data to the Chart
    ...
    #private
    #loops thtu the list of data and finds min and max
    ...
    def __draw_data(self):
        ...
        for (x, y) in self.list_of_emlements:
            ...
            self.__draw_p_line(px1,py1,px2,py2)
            px1=px2
            py1=py2
    ...
    # private
    # Find the bounds of the X y axis so all the is displayed optimally
